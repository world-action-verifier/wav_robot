...
class Logger:
    ...
    def video_file(self, name, path, fps=16):
        self._video_files[name] = (str(path), int(fps))
    ...
    def write(self, step, fps=False, flush=False):
        ...
        for name, (path, video_fps) in self._video_files.items():
            wandb.log({name: wandb.Video(path, fps=video_fps)}, step=step)

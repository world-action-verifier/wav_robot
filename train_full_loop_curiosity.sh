...
# Example (auto old/new from online checkpoints):
#   TASKS="lift" bash train_full_loop_curiosity.sh
#
# Example (explicit old/new override):
#   TASKS="lift" \
#   WM_NEW_CKPT_PATH="path/to/new.pt" \
#   WM_OLD_CKPT_PATH="path/to/old.pt" \
#   bash train_full_loop_curiosity.sh
#
# Example (EMA old from old-init + new):
#   TASKS="lift" \
#   WM_NEW_CKPT_PATH="path/to/new.pt" \
#   WM_OLD_INIT_CKPT_PATH="path/to/old_init.pt" \
#   EMA_GAMMA=0.99 \
#   bash train_full_loop_curiosity.sh
...
# Optional: if unset, trainer uses current online checkpoint as `new`.
WM_NEW_CKPT_PATH="${WM_NEW_CKPT_PATH:-}"

# Optional:
# - WM_OLD_CKPT_PATH: explicit old checkpoint.
# - WM_OLD_INIT_CKPT_PATH: old init checkpoint to build EMA old with EMA_GAMMA.
...
for TASK in ${TASKS}; do
  ...
  echo "=== Launching WM-only training for task: ${TASK} | run: ${RUN_NAME} ==="
